...
import dev.joseluisgs.factories.PersonaFactoy
...
fun main() {
    ...
    val equipoCampo: Array<JugadorCampo> = Array(10) { PersonaFactoy.getJugadorRandom() }
    val equipoPersonas: Array<Persona> = Array(20) { PersonaFactoy.getPersonaRandom() }

    for (persona in equipoCampo) {
        dimeQueEres(persona)
    }

    println()
    for (persona in equipoPersonas) {
        dimeQueEres(persona)
    }

    // Cuidado lo que devolvemos en las factorias y como lo tratamos
    val portero2 = PersonaFactoy.getJugador("Portero 2", LocalDate.now(), 1, JugadorCampo.Posicion.PORTERO)
    (portero2 as Portero).parar()
    val portero3 = PersonaFactoy.getPortero("Portero 3", LocalDate.now(), 1)
    portero3.parar()

    val defensa02 = PersonaFactoy.getJugador("Defensa 2", LocalDate.now(), 2, JugadorCampo.Posicion.DEFENSA)
    (defensa02 as Jugador).chutar()
    val defensa03 = PersonaFactoy.getDefensa("Defensa 3", LocalDate.now(), 3)
    defensa03.chutar()
